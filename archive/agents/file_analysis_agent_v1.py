...
async def file_analysis_agent(state: AssessmentState) -> AssessmentState:
    ...
    try:
        ...
        refinement_needed = state.get('strategy_refinement_needed')
        ...
        if refinement_needed and refinement_count < 3:
            # Re-strategizing based on feedback
            logger.info(f"Re-strategizing (attempt {refinement_count + 1}/3)")
            logger.info(f"Refinement reason: {refinement_needed.get('reason')}")
            
            # Get previous strategy
            previous_strategy = state.get('file_analysis_strategy')
            
            if not previous_strategy:
                logger.warning("No previous strategy found for refinement, creating new strategy")
                ...
                # Refine strategy with feedback
                strategy = await llm_service.refine_file_splitting_strategy(
                    structure_info=structure_info,
                    platform=platform,
                    file_path=file_path,
                    previous_strategy=previous_strategy,
                    refinement_feedback=refinement_needed
                )
            
            # Store refined strategy in state
            state['file_analysis_strategy'] = strategy
            state['strategy_refinement_count'] = refinement_count + 1
            state['strategy_refinement_needed'] = None  # Clear feedback flag
            state['status'] = 'file_analysis_complete'
            
            logger.info(f"Completed file analysis agent - refined strategy with {len(strategy.get('chunks', []))} chunks")
            return state
            
        elif refinement_needed and refinement_count >= 3:
            # Max refinements reached
            logger.error("Max strategy refinements (3) reached, cannot refine further")
            state['errors'] = state.get('errors', []) + [
                "Strategy refinement failed after 3 attempts"
            ]
            state['status'] = 'file_analysis_complete'
            return state
        else:
            # Normal initial strategy creation
