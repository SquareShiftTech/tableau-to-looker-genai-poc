...
INGEST_WORKSHEETS = """
INSERT INTO worksheets (workbook_id, name, viz_definition)
SELECT DISTINCT
    w.id,
    ws->>'@name',
    ws->'table'
...
class Database:
    ...
    def get_counts(self) -> dict:
        ...
        tables = ["workbooks", "datasources", "fields", "worksheets", "dashboards", "dashboard_components", "actions"]
        counts = {}
        with self.conn.cursor() as cur:
            for table in tables:
    ...
    def truncate_all(self):
        ...
        with self.conn.cursor() as cur:
            cur.execute("""
                TRUNCATE TABLE actions, dashboard_components, dashboards, worksheets, 
                fields, parameters, datasources, workbooks, raw_tableau_files, json_schema 
                CASCADE;
            """)
            self.conn.commit()
