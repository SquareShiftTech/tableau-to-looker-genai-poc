"""
Agent 2: Complexity Analyzer (Simple Workflow)

This agent asks Agent 1 intelligent questions to assess migration complexity.
It demonstrates the conversational multi-agent architecture with retry logic.
"""
import sys
import time
...
from config.settings import MAX_RETRIES_AGENT_2, RETRY_DELAY_SECONDS


def agent_2_complexity_analyzer(state: AnalysisState) -> AnalysisState:
    """
    Agent 2: Complexity Analyzer (Simple Workflow with Retry)
    
    Analyzes complexity for migration assessment by asking Agent 1 questions.
    Includes retry logic for failed queries.
    ...
    """
    print("\n" + "="*70)
    print("🔍 AGENT 2: COMPLEXITY ANALYZER (WITH RETRY)")
    print("="*70)
    ...
    print(f"🔄 Max Retries per Question: {MAX_RETRIES_AGENT_2}")
    print("\n" + "-"*70)
    ...
    # Ask each question to Agent 1 with retry
    for i, question in enumerate(questions, 1):
        print(f"\nQuestion {i}/{len(questions)}")
        
        # Retry loop for this question
        answer = None
        for attempt in range(1, MAX_RETRIES_AGENT_2 + 1):
            try:
                answer = query_tableau_expert(state, question, asking_agent="agent_2_complexity")
                
                # Validate answer is meaningful
                if answer and len(answer) > 10 and "ERROR" not in answer:
                    if attempt == 1:
                        analysis["retry_stats"]["successful_first_try"] += 1
                    else:
                        analysis["retry_stats"]["required_retry"] += 1
                        print(f"   ✅ Success on attempt {attempt}")
                    break  # Success!
                else:
                    if attempt < MAX_RETRIES_AGENT_2:
                        print(f"   ⚠️  Invalid answer on attempt {attempt}, retrying...")
                ...
                if attempt < MAX_RETRIES_AGENT_2:
                    ...
                    print(f"   ❌ All retries failed for question {i}")
                    answer = f"Failed after {MAX_RETRIES_AGENT_2} attempts"
                    analysis["retry_stats"]["failed"] += 1
